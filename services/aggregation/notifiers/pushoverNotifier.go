...
func (notifier *pushoverNotifier) OutputMessages(messages ...common.OutputMessage) error {
	...
	msgString := ""
	highestMessageOutputType := common.MessageOutputType(0)
...
func createMessageString(
	...
	if len(msg.ProblemEncountered) == 0 {
		return fmt.Sprintf("%s %s\n\n",
			iconString, msg.Identifier)
	}

	return fmt.Sprintf("%s %s: %s\n\n",
		iconString, msg.Identifier, msg.ProblemEncountered)
}

func getIconString(message common.OutputMessage) string {
	switch message.Type {
	case common.InfoMessageOutputType:
		return "✅"
	case common.ErrorMessageOutputType:
		return "🚨"
	case common.WarningMessageOutputType:
		return "⚠️"
	default:
		return ""
	}
...
func createTitle(maxMessageOutputType common.MessageOutputType, executor string) string {
...
func (notifier *pushoverNotifier) pushNotification(msgString string, title string) error {
	...
	if !common.IsHttpStatusCodeSuccess(statusCode) {
